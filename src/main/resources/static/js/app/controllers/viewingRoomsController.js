    ...
    function viewingRoomsController($scope, $location, $routeParams, viewingRoomsService) {
        ...
        $scope.closeSeats = function(seatSegment)
        {
            ...
            for (let index = 0; index < $scope.seats.length; index++) {
                ...
                {
                    if ($scope.ids.includes( $scope.seats[index].id ) )
                        toastr.success("Seat wtih id " + $scope.seats[index].id +" can not be closed because it is already taken");
                    else
                    {
        ...
        $scope.makeSeatLayout = function() {
            ...
            for (j = 0; j < $scope.seats.length; j++) {
                ...
                var node = {
                    "type": 1,
                    "uniqueName": $scope.seats[j].segment,
                    "displayName": $scope.seats[j].segment,
                    "color":"#BB1F31",
                    "selected": 0
                };
                row["nodes"].push(node);
